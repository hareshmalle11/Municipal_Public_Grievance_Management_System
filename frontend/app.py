...
if not is_connected:
    st.error("⚠️ System Offline: Cannot connect to Municipal Backend API at http://127.0.0.1:8000. Please start the backend service first by running `uvicorn main:app --reload` inside the `backend` folder.")
    st.stop()
